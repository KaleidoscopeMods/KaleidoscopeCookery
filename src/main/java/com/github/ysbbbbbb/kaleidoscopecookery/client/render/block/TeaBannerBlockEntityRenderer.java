...
public class TeaBannerBlockEntityRenderer implements BlockEntityRenderer<TeaBannerBlockEntity> {
    ...
    public void render(TeaBannerBlockEntity teaBanner, float partialTick, PoseStack poseStack,
        ...
        ResourceLocation baseTexture = new ResourceLocation(MOD_ID, "textures/entity/banner/" + teaBanner.getColor().getName() + ".png");
        VertexConsumer baseBuffer = buffer.getBuffer(RenderType.entityCutoutNoCull(baseTexture));
        if (wallMounted) {
            this.wallBanner.renderToBuffer(poseStack, baseBuffer, packedLight, packedOverlay, 1.0F, 1.0F, 1.0F, 1.0F);
        } else {
            this.normalBanner.renderToBuffer(poseStack, baseBuffer, packedLight, packedOverlay, 1.0F, 1.0F, 1.0F, 1.0F);
        }
        ...
        ResourceLocation patternTexture = new ResourceLocation(MOD_ID, "textures/entity/pattern/" + patternName + ".png");
        ...
        this.patternModel.renderToBuffer(poseStack, patternBuffer, packedLight, packedOverlay, 1.0F, 1.0F, 1.0F, 1.0F);
        poseStack.popPose();
    }
}
